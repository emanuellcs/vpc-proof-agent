...
- **Probe engine** (`internal/probe`): nine probes for metadata, VPC and subnet ownership, default route, DNS, outbound HTTPS, public-IP consistency, system resources, and clock skew; a runner with per-probe and global timeouts.
...
- **Tooling and operations**: a Makefile (build, test, lint, e2e, LocalStack targets), strict golangci-lint configuration, GitHub Actions CI, an idempotent LocalStack provisioning script, and a hardened systemd unit.
