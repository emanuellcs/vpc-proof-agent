...
func TestValidate(t *testing.T) {
	tests := []struct {
		...
		{name: "tls cert without key", mutate: func(c *Config) { c.Server.TLSCertFile = "cert.pem" }, want: "tls_cert_file and server.tls_key_file"},
		{name: "tls key without cert", mutate: func(c *Config) { c.Server.TLSKeyFile = "key.pem" }, want: "tls_cert_file and server.tls_key_file"},
		{name: "tls cert and key valid", mutate: func(c *Config) { c.Server.TLSCertFile = "cert.pem"; c.Server.TLSKeyFile = "key.pem" }, want: ""},
		{name: "history max entries zero", mutate: func(c *Config) { c.History.MaxEntries = 0 }, want: "history.max_entries"},
		{name: "history flush interval zero", mutate: func(c *Config) { c.History.FlushInterval = 0 }, want: "history.flush_interval"},
